    ...
    return this.http
      ...
      .pipe(
        map((response) => {
          ...
          const totalRaw =
            response.total_items ?? response.total_count ?? response.total ?? data.length;
            ...
            pagination: {
              ...
              total_pages:
                Number.isFinite(normalizedPageSize) && normalizedPageSize > 0
                  ? Math.ceil(
                      (Number.isFinite(normalizedTotal) ? normalizedTotal : data.length) /
                        normalizedPageSize,
                    )
                  : 0,
            },
